...
def profile_task(request: SolveRequest, thresholds: dict[str, float]) -> TaskProfile:
    ...
    if request.validation is not None:
        ...
        if request.validation.kind == "reference_json":
            required.add("structured_output")
    if request.task_type == "vision":
        required.add("vision")
    grounding = (
        request.task_type in {"research", "grounded_research", "factual_research"}
        or request.freshness_required
        or bool(re.search(r"\b(latest|current|sources?|citations?|research)\b", task))
    )
    return TaskProfile(
